...
- [~] `security_deposits` — held deposit per lease, escrow status, forfeiture → landowner payout (RLS parties+staff). *Decision settled: separate charge+refund (Stripe auth-hold rejected — 7-day window too short for leases).*
  ...
  - [ ] **Slice 2+ — lease-deposit flow** (needs Stripe keys + lease-activation hook): `SecurityDepositService` (record/hold/release/forfeit under `db.system`), deposit charge at lease activation, webhook handling, forfeiture→landowner payout, admin + member UI.
