...
- [~] `security_deposits` — held deposit per lease, escrow status, forfeiture → landowner payout (RLS parties+staff). *Decision settled: separate charge+refund (Stripe auth-hold rejected — 7-day window too short for leases).*
  - [x] **Slice 1 — schema + model + RLS forgery-defense** (migration `2026_06_20_000003`, `App\Models\Billing\SecurityDeposit`). System-authored, runtime-read-only: SELECT for parties+staff `TO ah_runtime`, no write policy (only `ah_system` writes). Regression: `tests/Feature/Security/SecurityDepositRlsWriteTest` (7 tests). `db04_billing.md` marked BUILT + added to RLS list.
  - [x] **Slice 2 — lease-deposit lifecycle** (capture/release/forfeit): `App\Services\Billing\SecurityDepositService` (`amountDueCents`/`forLease`/`createCheckoutSession`/`recordHeldFromCheckout`/`release`/`forfeit`) + `StripeService::{createDepositCheckoutSession,refundPaymentIntent}`. **Collection = in-portal hosted Checkout** (member "Pay Deposit" on `Member/Lease.tsx` → `POST /member/leases/{lease}/deposit` → Stripe Checkout `mode=payment`; row authored by webhook under `ah_system`, never the runtime path — SEC-045). `ProcessStripeWebhook` `checkout.session.completed` `mode=payment`+`metadata.purpose=security_deposit` branch records the held row (idempotent on PaymentIntent). Release refunds full remaining balance; forfeit records amount+reason and returns any un-forfeited remainder to the lessee. Admin: read-only `SecurityDepositResource` (Billing group) with release/forfeit header actions. Tests: `SecurityDepositServiceTest` (12) + webhook deposit branch (2 in `ProcessStripeWebhookTest`). `db04_billing.md` updated.
  - [x] **Slice 3 — forfeiture payout** (BUILT 2026-06-25): `forfeit()` now disburses the forfeited amount to the landowner (deposit's `payee_user_id`) via `PayoutService::disburse` — best-effort: the forfeiture state is recorded first, so a Stripe failure or a landowner who hasn't onboarded a Connect account never rolls it back (cash stays captured for a later retry; logged). The platform tier fee is withheld like any payout. Tests: `SecurityDepositServiceTest` +2 (disburses to onboarded landowner; defers when not onboarded).
...
- [~] `App\Services\Billing\PayoutService` — Stripe Connect disbursement scheduling, platform fee calculation per landowner tier. **Slice 1 BUILT (2026-06-25):** fee engine + disbursement core + Connect SDK wrappers + RLS hardening, Stripe mocked (no live keys needed).
  ...
  - [x] **`PayoutService`** — `platformFeePct` (promo claim → subscription locked version → free-tier plan precedence, reads `plan_versions.platform_fee_pct`); `quote` (gross → fee/net); `connectAccount`/`canReceivePayouts`; `disburse` (transfers net, records `payouts` row `paid` under ah_system, audits — see 5.5 Sub-slice C: the Transfer is synchronous and has no settlement webhook, so the prior `in_transit` was a dangling status). Tests: `tests/Feature/Billing/PayoutServiceTest` (5, Stripe mocked). Full billing suite 127 green.
  - [~] **Slice 2 — onboarding front door + forfeiture disbursement (BUILT 2026-06-25, Stripe mocked):**
    - [x] **Landowner Connect onboarding** — `App\Http\Controllers\Member\PayoutController` (`connect`/`refresh`/`return`) + routes `POST /member/payouts/connect`, `GET /member/payouts/{refresh,return}` (connect/refresh under `db.system` — the `stripe_accounts` INSERT is system-authored per SEC-055; landowner-only guard). `PayoutService::startOnboarding` mints/persists the Connect account once and returns the hosted onboarding URL; `onboardingState` shapes status for the UI. Member UI: a "Payouts" card in the **My Properties** sidebar of `Member/Profile/Hunter.tsx` (Set up / Finish / Active states + `?payouts=` return banner); `ProfileController` passes `payouts` + `payouts_status` for landowners. Tests: `PayoutServiceTest` +4 (onboarding state ×2, start creates/returns link, reuses existing) + `tests/Feature/Member/PayoutOnboardingTest` (2 — landowner handoff, non-landowner 403).
...
> **Status (2026-06-27):** lease-rent collection landed via **destination charges + `on_behalf_of`** (branch `feature/connect-lease-payments`, Stripe mocked). The collection model — chosen over separate post-clear disbursement — keeps the platform as merchant of record (owns refunds/disputes) while the landowner's net is transferred at charge time and settlement + the 1099-K attribute to them. No `DisburseLandowernPayout` job and no hold-period scheduler are needed: there is no platform-held balance to schedule out.

- [x] Landowner Stripe Connect Express onboarding — creates account, tracks `charges_enabled` and `payouts_enabled` (built in 5.2 Slice 2; `PayoutController` + `Member/Profile/Hunter.tsx` payouts card).

**Money model (one charge):** `gross = rent_balance + processing_surcharge` (customer pays); `application_fee = round(rent_balance × tier platform_fee_pct) + surcharge` (platform revenue, `payment_intent_data.application_fee_amount`); `net = gross − application_fee = rent_balance − tier_fee` (auto-transferred to the landowner via `transfer_data[destination]`). Gate is **`charges_enabled`** (a destination requires the connected account to take the transfer), not `payouts_enabled`.

- [x] **Sub-slice A — destination-charge lease-rent collection (core):**
  - [x] `StripeService::createConnectCheckoutSession` (mode=payment; `application_fee_amount` + `transfer_data[destination]` + `on_behalf_of` = landowner), `refundDestinationCharge` (`reverse_transfer` + `refund_application_fee` — fixes the refund-fee clawback for this payment type), `chargeAndTransferForPaymentIntent` (captures the auto-created charge + transfer ids).
  - [x] `billing/2026_06_27_000001_create_lease_payments_table` — single source of truth per charge (gross/surcharge/application_fee/net cents, status collected/refunded/partially_refunded, PI/charge/transfer ids, unique on PI). No `deleted_at` (financial record resolves via status). SEC-045 RLS: enabled + one `FOR SELECT TO ah_runtime` policy (two parties + staff), no write policy → system-authored.
  - [x] `App\Models\Billing\LeasePayment` (extends `BaseModel`; Stripe ids `$hidden`; cross-DB `getPayer`/`getPayee`/`getLease`/`leaseLabel`).
  - [x] `App\Services\Billing\LeasePaymentService` — `balanceDueCents` (total − booking-deposit-credited − prior rent), `quote`, `createCheckoutSession` (gated on `charges_enabled`, throws a domain error otherwise), `recordCollectedFromCheckout` (system-authored, idempotent on PI, audits gross/fee/net only — never card data), `refund`.
  - [x] Webhook — `ProcessStripeWebhook::checkoutCompleted` branch `mode=payment && purpose=lease_payment` → `recordCollectedFromCheckout` (the destination transfer is auto-created, so no extra event handler).
  - [x] Member portal — `POST /member/leases/{lease}/lease-payment` + `GET …/return` (under `db.system`, reconciles before the webhook lands); `MemberController::{payLeaseBalance,leasePaymentReturn}` + a `lease_payment` prop; a **Lease Balance** card in `Member/Lease.tsx` (amount due + surcharge small print; "Awaiting landowner payout setup" when `charges_enabled` is false; payment history).
  - [x] Onboarding gate copy keyed on `charges_enabled` (lease payments receivable) vs `payouts_enabled` (bank settlement) in `Member/Profile/Hunter.tsx`.
  - [x] Admin Filament — `LeasePaymentResource` (read-only, nav group Billing, cross-DB party names) + View **Refund** action → `LeasePaymentService::refund`.
- [x] **Sub-slice B — booking deposit → destination charge:** `BookingDepositService::createCheckoutSession` now uses `createConnectCheckoutSession` (gated on `charges_enabled`), `recordCollectedFromCheckout` records the row `disbursed` with the auto-created transfer id; `billing/2026_06_27_000002_add_connect_fields_to_booking_deposits` adds `stripe_account_id`/`application_fee_cents`/`net_cents`/`stripe_transfer_id`. Clears the deferred-disbursement debt for go-forward deposits (pre-existing collected deposits stay platform-held — manual reconciliation).
...
- [x] **Webhook upsert** — `ProcessStripeWebhook` branches for `invoice.created`/`finalized`/`paid`/`voided` (→ `upsertInvoiceProjection`), `invoice.payment_failed` (projects + marks `past_due`), `charge.refunded` (→ `chargeRefunded`); one `updateOrCreate` per event (no full-table refresh); runs under `ah_system` (existing). **Probe-confirmed dahlia field paths:** the invoice has NO top-level `subscription` — subscriber + plan version ride in `invoice.parent.subscription_details.{subscription,metadata.user_id}`; subscription-invoices-only (no `parent` → skipped). **Refund mapping:** `charge.refunded` carries only `payment_intent` (the dahlia API dropped both `charge.invoice` AND `payment_intent.invoice`), so the PI is captured on the row at `invoice.paid` time via new `StripeService::invoicePaymentIntentIdFor()` (one expanded retrieve) + new column (migration `2026_06_20_000002`, `stripe_payment_intent_id`, hidden); the refund event then maps by stored PI. **Latent bug fixed in the same handler:** `invoicePaymentFailed` was reading the always-null top-level `subscription` → dunning `past_due` never fired under dahlia; now resolves via `parent.subscription_details`. Tests: `ProcessStripeWebhookTest` +6 (paid→projection+PI capture, idempotent upsert advances status, voided, non-subscription ignored, payment_failed→past_due+projects, charge.refunded full→partial); 17 passed
- [x] **Reconciliation backstop** — `App\Jobs\Billing\ReconcileStripeInvoices` (daily 04:00 via `routes/console.php`, `default` queue → `ah_system`, tries=1/timeout=600, `$lookbackDays=45`). Re-pulls recent subscription invoices and upserts via the shared `App\Services\Billing\StripeInvoiceProjector` — extracted so webhook + reconcile can't drift on dahlia field paths (webhook now delegates to it too). Refund totals reconciled by summing in-window `Charge.amount_refunded` keyed by PaymentIntent (event payloads can't map charge→invoice under dahlia); out-of-window charges pass `refund=null` so webhook-set refund fields aren't clobbered. `StripeService::reconcileInvoiceProjections()` does the paginated pull (`expand=['data.payments']`, skips non-subscription invoices). Tests: `StripeInvoiceProjectorTest` (6, mapping) + `ReconcileStripeInvoicesTest` (3, wiring) + existing webhook suite — **26 passed**. Live test-mode reconcile probe pulled 1 real invoice end-to-end (PI captured, user mapped)
...
- [x] **Backfill command** — `App\Console\Commands\BackfillInvoiceProjection` (`php artisan billing:backfill-invoice-projection [--days=730]`, default 2-yr lookback). Thin console wrapper over the same `StripeService::reconcileInvoiceProjections()` path the daily job uses (idempotent, upserts on stripe_invoice_id), runs under ah_system (console) so writes succeed; fails fast with exit 1 when Stripe is unconfigured, reports rows upserted + total projection count. Tests: `BackfillInvoiceProjectionTest` (3, StripeService mocked) — passed. Live test-mode run pulled 1 invoice → projection holds 1 row. **Run this before the read-path cutover and confirm the projection is populated.**
