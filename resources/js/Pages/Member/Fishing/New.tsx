...
export default function FishingNew({ leases, species, store_url, index_url }: Props) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    length_inches: '',
    weight_lbs: '',
    catch_and_release: false as boolean,
    notes: '',
  ...
  const { queue } = useOfflineSubmit('fishing', index_url)
  const online = useOnline()

  function submit(e: React.FormEvent) {
    e.preventDefault()
    if (!navigator.onLine) {
      const label = species.find(s => s.value === data.species_code)?.label ?? 'Catch'
      ...
      <div style={{ minHeight: '100vh', background: '#faf7f2' }}>
        <div style={{ background: INK, borderBottom: '1px solid #1a2e28' }}>
          <div style={{ maxWidth: '560px', margin: '0 auto', padding: '0 16px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <a href={index_url} style={{ fontFamily: MONO, fontSize: '10px', letterSpacing: '.1em', textTransform: 'uppercase', color: '#6b9e8f', textDecoration: 'none' }}>Fishing Log</a>
          </div>
        </div>
        ...
        <div style={{ maxWidth: '560px', margin: '0 auto', padding: '40px 16px 64px' }}>
          <div style={{ marginBottom: '24px' }}>
            ...
            <h1 style={{ fontFamily: DISPLAY, fontSize: '28px', fontWeight: 400, color: INK, margin: 0, lineHeight: 1.1 }}>Log a Catch</h1>
          </div>
          ...
          {leases.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid #e5e0d8', borderRadius: '4px', padding: '28px 24px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '14px', color: '#6b5e50', lineHeight: 1.5, margin: '0 0 16px' }}>
                You need an active lease to log a catch. If you're a guest on someone's lease, ask the lease holder to add and approve you.
              </p>
            ...
            <form onSubmit={submit} style={{ background: '#fff', border: '1px solid #e5e0d8', borderRadius: '4px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              {!online && (
                <div style={{ background: '#fef6ee', border: '1px solid #f0c9a8', borderRadius: '3px', padding: '11px 14px', fontFamily: MONO, fontSize: '11px', color: '#9a4a1e', lineHeight: 1.5 }}>
                  You're offline. This catch will be saved on your device and synced automatically when you're back on signal.
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 160px' }}>
                  ...
                  <input type="date" max={today()} value={data.catch_date} onChange={e => setData('catch_date', e.target.value)} style={inputStyle} />
                  {errors.catch_date && <div style={errStyle}>{errors.catch_date}</div>}
                </div>
                <div style={{ flex: '1 1 120px' }}>
                  <label style={labelStyle}>Time <span style={{ color: '#a89874' }}>(opt)</span></label>
                  <input type="time" value={data.catch_time} onChange={e => setData('catch_time', e.target.value)} style={inputStyle} />
                  {errors.catch_time && <div style={errStyle}>{errors.catch_time}</div>}
                </div>
              </div>

              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 120px' }}>
                  <label style={labelStyle}>Length in. <span style={{ color: '#a89874' }}>(opt)</span></label>
                  <input type="number" step="0.01" min="0" value={data.length_inches} onChange={e => setData('length_inches', e.target.value)} style={inputStyle} />
                  {errors.length_inches && <div style={errStyle}>{errors.length_inches}</div>}
                </div>
                <div style={{ flex: '1 1 120px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                <input type="checkbox" checked={data.catch_and_release} onChange={e => setData('catch_and_release', e.target.checked)} />
                <span style={{ fontSize: '13px', color: '#6b5e50' }}>Catch &amp; release</span>
              </label>

              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                <input type="checkbox" checked={data.is_public} onChange={e => setData('is_public', e.target.checked)} />
                <span style={{ fontSize: '13px', color: '#6b5e50' }}>Show this catch on my public gallery <span style={{ color: '#a89874' }}>(location is never shown)</span></span>
              </label>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                <button type="submit" disabled={processing} style={{ flex: 1, padding: '14px', background: BLAZE, color: '#fff', border: 'none', borderRadius: '3px', fontFamily: MONO, fontSize: '13px', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', cursor: processing ? 'not-allowed' : 'pointer', opacity: processing ? 0.7 : 1 }}>
                  {processing ? 'Logging…' : online ? 'Log Catch' : 'Save Offline'}
                </button>
