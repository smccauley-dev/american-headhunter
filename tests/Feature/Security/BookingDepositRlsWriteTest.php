...
/**
 * booking_deposits is system-authored and runtime-read-only.
 *
 * It mirrors security_deposits (and invoices/payments/payouts): RLS is enabled with
 * a single FOR SELECT policy TO ah_runtime (the two parties + staff) and NO write
 * policy. ah_runtime inherits a table-level DML grant via ALTER DEFAULT PRIVILEGES,
 * so this test proves that grant is inert for writes — RLS default-denies every
 * INSERT/UPDATE — while reads stay scoped to the lessee, the landowner, and staff.
 * Only ah_system (BYPASSRLS — the booking-deposit webhook, deferred payout job,
 * Filament admin) may author these rows.
 *
 * Postgres-only. Skips cleanly when an ah_runtime connection is unavailable.
 */
class BookingDepositRlsWriteTest extends TestCase
{
    private const RUNTIME = 'billing_booking_rls_write_test';
    ...
    {
        DB::connection('billing')->table('booking_deposits')
    ...
    /** A complete booking-deposit row payload. */
    ...
    /** Seed a booking-deposit row directly (owner connection, bypasses RLS). */
    private function seedDeposit(): string
    {
        $row = $this->depositRow();
        DB::connection('billing')->table('booking_deposits')->insert($row);

        return $row['id'];
    }

    public function test_rls_is_enabled_on_booking_deposits(): void
    {
        $row = DB::connection('billing')->selectOne(
            "SELECT relrowsecurity FROM pg_class WHERE relname = 'booking_deposits'"
        );
        $this->assertTrue((bool) $row->relrowsecurity, 'RLS must be enabled on booking_deposits.');
    }
    ...
    public function test_payer_can_read_own_deposit(): void
    {
        ...
        $this->assertSame(1, DB::connection(self::RUNTIME)->table('booking_deposits')
    ...
    public function test_unrelated_user_cannot_read_the_deposit(): void
    {
        ...
        $this->assertSame(0, DB::connection(self::RUNTIME)->table('booking_deposits')
            ->where('payer_user_id', $this->payerId)->count());
    }
    ...
    public function test_runtime_cannot_insert_deposit(): void
    {
        ...
        DB::connection(self::RUNTIME)->table('booking_deposits')->insert($this->depositRow());
    }
    ...
    public function test_runtime_cannot_update_deposit(): void
    {
        ...
        // affected (system-authored: a party can never mutate their booking deposit).
        $affected = DB::connection(self::RUNTIME)->table('booking_deposits')
            ->where('id', $id)->update(['status' => 'disbursed']);

        $this->assertSame(0, $affected, 'A party must not be able to mutate their booking deposit.');
    }
}
