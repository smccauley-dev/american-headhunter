...
> - ✅ **`security_deposits`** — BUILT (migration `2026_06_20_000003`); adds `security_deposits` to the RLS list. Slices 1–3 done: schema + model + RLS forgery-defense, the capture/release/forfeit `SecurityDepositService` with in-portal hosted-Checkout collection, webhook authoring, and admin + member UI, **and** forfeiture→landowner payout via Stripe Connect (`forfeit()` disburses the forfeited amount through `PayoutService`, best-effort).
