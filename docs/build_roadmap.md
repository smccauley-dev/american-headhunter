...
- [~] `security_deposits` — held deposit per lease, escrow status, forfeiture → landowner payout (RLS parties+staff). *Decision settled: separate charge+refund (Stripe auth-hold rejected — 7-day window too short for leases).*
  ...
  - [ ] **Slice 3 — forfeiture payout** (deferred, needs Stripe Connect/`PayoutService`): the forfeited amount currently stays captured on the platform — `forfeit()` records state + reason + audit but does **not** disburse to the landowner. Wire the landowner transfer when Connect ships.
...
- [~] `App\Services\Billing\PayoutService` — Stripe Connect disbursement scheduling, platform fee calculation per landowner tier. **Slice 1 BUILT (2026-06-25):** fee engine + disbursement core + Connect SDK wrappers + RLS hardening, Stripe mocked (no live keys needed).
  ...
  - [ ] **Slice 2 (deferred — needs Stripe test keys for live verification):** landowner Connect onboarding controller + route + member UI; `account.updated` webhook branch → upsert `stripe_accounts` flags; `DisburseLandownerPayout` job (post-clear hold period); `security_deposits` slice 3 (forfeiture → `PayoutService::disburse`).
