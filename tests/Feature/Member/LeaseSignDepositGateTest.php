...
/**
 * Pay-then-sign gate (LeaseSignController::sign). The lessee's signature is the
 * one that activates the lease, so when the listing requires a refundable
 * deposit the lessee cannot sign until it is held. This proves the server-side
 * enforcement — the Sign page UI only mirrors it.
 *
 * SecurityDepositService is mocked so the gate is exercised without a Stripe
 * round-trip or a property/listing fixture (the amount derivation is covered by
 * SecurityDepositServiceTest). The lease, esign request and signers are real.
 */
class LeaseSignDepositGateTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        DB::connection('identity')->table('users')->insert([
            'id'            => $this->userId,
            'email'         => "lessee-gate-{$this->userId}@test.invalid",
    ...
    /** Bind a deposit service that reports a $500 deposit due, held or not. */
    private function mockDeposit(bool $held): void
    {
        $mock = Mockery::mock(SecurityDepositService::class);
        $mock->shouldReceive('amountDueCents')->andReturn(50000);
        $mock->shouldReceive('forLease')->andReturn(
            $held ? new SecurityDeposit(['status' => 'held', 'amount_cents' => 50000]) : null,
        );

        $this->app->instance(SecurityDepositService::class, $mock);
    }
    ...
    public function test_lessee_cannot_sign_until_the_deposit_is_held(): void
    {
        $this->mockDeposit(held: false);
        ...
        $this->assertSame('pending', $this->lesseeSignerStatus(), 'signature must not be recorded while the deposit is unpaid');
    }

    public function test_lessee_can_sign_once_the_deposit_is_held(): void
    {
        $this->mockDeposit(held: true);
        ...
        $this->assertSame('signed', $this->lesseeSignerStatus(), 'signature should be recorded once the deposit is held');
    }
}
