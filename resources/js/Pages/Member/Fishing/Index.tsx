...
export default function FishingIndex({ catches, new_url, harvest_url }: Props) {
          ...
          {catches.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid #e5e0d8', borderRadius: '4px', padding: '32px 24px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '14px', color: '#6b5e50', lineHeight: 1.5, margin: '0 0 16px' }}>
                Fishing the water on your lease? Log what you land — species, length, and weight — and keep a running record of the season.
              </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {catches.map(c => (
                <div key={c.id} style={{ background: '#fff', border: '1px solid #e5e0d8', borderRadius: '4px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                  <div>
                    <div style={{ fontFamily: DISPLAY, fontSize: '17px', color: INK, lineHeight: 1.2 }}>
                      {c.species}
                      {c.length_inches != null && (
                        <span style={{ fontFamily: MONO, fontSize: '11px', color: BLAZE, marginLeft: '8px' }}>{c.length_inches}&quot;</span>
                      )}
                    </div>
                    <div style={{ fontSize: '13px', color: '#6b5e50', marginTop: '3px' }}>
                      {c.property_title}
                      {c.weight_lbs != null && <> · {c.weight_lbs} lbs</>}
                      {c.catch_and_release && <> · released</>}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: MONO, fontSize: '12px', color: '#6b5e50' }}>{c.catch_date}</div>
                    {c.is_public && (
